...
class phpSniff_core { // initialize some vars
		var $_browser_info = array ('ua' => '', 'browser' => 'Unknown', 'version' => 0, 'maj_ver' => 0, 'min_ver' => 0, 'letter_ver' => '', 'javascript' => '0.0', 'platform' => 'Unknown', 'os' => 'Unknown', 'ip' => 'Unknown', 'cookies' => 'Unknown', // remains for backwards compatability
	'ss_cookies' => 'Unknown', 'st_cookies' => 'Unknown', 'language' => '', 'long_name' => '', 'gecko' => '', 'gecko_ver' => '');

	var $_feature_set = array ('html' => true, 'images' => true, 'frames' => true, 'tables' => true, 'java' => true, 'plugins' => true, 'iframes' => false, 'css2' => false, 'css1' => false, 'xml' => false, 'dom' => false, 'wml' => false, 'hdml' => false);

	var $_quirks = array ('must_cache_forms' => false, 'avoid_popup_windows' => false, 'cache_ssl_downloads' => false, 'break_disposition_header' => false, 'empty_file_input_value' => false, 'scrollbar_in_way' => false);
	...
	/**
	* Modified by Massimiliano Balestrieri
	* Add the check of server port
	* Modified the logic. The check run only the first request
	* Removed the Querystring
	* Link to Deskweb Session Object
	*/
		...
		if ($this->_check_cookies) {
			...
			//if (!$fp) {
				...
				if (getenv('SERVER_PORT') != "80")
					$server = getenv('SERVER_NAME').":".getenv('SERVER_PORT');
				else
				...
				$js .= 'function setCookie(c_name,value,expiredays){var exdate=new Date();exdate.setDate(exdate.getDate()+expiredays);document.cookie=c_name+ "=" +escape(value)+((expiredays==null) ? "" : ";expires="+exdate);}';
				$js .= 'setCookie("js",1,365);';
				$js .= 'location.href = "http://'.$server.$script_path.'"; ';
				$js .= '</script>';
				$js .= '<noscript>';
				$js .= 'Il tuo browser non supporta javascript. Deskweb verrà avviato in modalità accessibile. Aggiorna o <a href="http://'.$server.$script_path.'">clicca qui</a>';
				$js .= '</noscript>';
				echo $js;
				
				$location = 'http://'.$server.$script_path; //. ($QS == '' ? '' : '?'.$QS);
				//echo $location;
				...
				$this->_set_browser('st_cookies', isset ($cookies['phpSniff_stored']) ? 1 : 0);
				// delete the old cookies
				//setcookie('phpSniff_session', '', 0, '/');
				//setcookie('phpSniff_stored', '', 0, '/');
				//phpSniff_core :: debug_cookie();
				//echo "qui";
				//exit;
			}
		}
	...
	/**
	 * Created for test by Massimiliano Balestrieri
	 */
	function debug_cookie() {

		echo "<pre>".@ print_r($this->_set_browser['ss_cookies'], true)."</pre><hr>";

		if (@ $this->_browser_info['ss_cookies'] == "false")
			echo "Cookie di Sessione non abilitati<hr>";
		else
			echo "Cookie di Sessione abilitati<hr>";

		echo "<pre>".@ $_COOKIE['phpSniff_session']."</pre><hr>";
		echo "<pre>".@ $_COOKIE['phpSniff_stored']."</pre><hr>";

		if (@ $this->_browser_info['st_cookies'] == "false")
			echo "Altri Cookie non abilitati<hr>";
		else
			echo "Altri Cookie abilitati<hr>";

		echo "<pre>".@ print_r($this, true)."</pre><hr>";
	}
