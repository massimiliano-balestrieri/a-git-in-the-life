...
/**
 * classe BugModelDeskWeb
 */
class DWBugModelDeskWeb {
	...
	/**
	 * costruttore
	 * prende da mebug statico di dao i dati che servono all'applicazione
	 */
	function DWBugModelDeskWeb() {
		global $request;
		$orderby = null;
		$where = null;
		$limit = null;
		...
		if (isset ($request->form['orderby']) || isset ($request->form['firstletterwhere']) || isset ($request->form['filterwhere'])) {
			$sql = "SHOW FIELDS FROM ".PREFIX_DB."bug";
		...
		$sql = "select * from ".PREFIX_DB."bug ".$where.$orderby.$limit;
		//echo $sql;
		$this->arr_bug = MySqlDao :: getDati($sql);
		for($x=0;$x<=sizeof($this->arr_bug)-1;$x++){
			$this->arr_bug[$x]['data'] = date("d/m/Y",strtotime($this->arr_bug[$x]['data']));	
		}
		
		$sql = "select count(*) as num_record from ".PREFIX_DB."bug ".$where.$orderby;
		$dati = MySqlDao :: getDati($sql);
