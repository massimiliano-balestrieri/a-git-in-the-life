...
/**
 * classe DWAddressbookModelDeskWeb
 */
class DWAddressbookModelDeskWeb {

	/**
	 * array dati weblets
	 */
	var $arr_addressbook = array ();
	/**
	 * numero di record
	 */
	var $num_record;
	 ...
	 */
	function DWAddressbookModelDeskWeb() {
		global $request;
		$orderby = null;
		$where = null;
		$limit = null;
		...
		if (isset ($request->form['orderby']) || isset ($request->form['firstletterwhere']) || isset ($request->form['filterwhere'])) {
			$sql = "SHOW FIELDS FROM ".PREFIX_DB."addressbook";
		...
		$sql = "select id_addressbook,nome,cognome,email,azienda,telefono_ufficio, telefono_cellulare from ".PREFIX_DB."addressbook ".$where.$orderby.$limit;
		//echo $sql;
		$this->arr_addressbook = MySqlDao :: getDati($sql);
		$sql = "select count(*) as num_record from ".PREFIX_DB."addressbook ".$where.$orderby;
		$dati = MySqlDao :: getDati($sql);
