...
/**
 * classe WishlistModelDeskWeb
 */
class DWWishlistModelDeskWeb {

	/**
	 * array dati weblets
	 */
	var $arr_wishlist = array ();
	/**
	 * numero di record
	 */
	var $num_record;
	...
	/**
	 * costruttore
	 * prende da mewishlist statico di dao i dati che servono all'applicazione
	 */
	function DWWishlistModelDeskWeb() {
		global $request;
		$orderby = null;
		$where = null;
		$limit = null;
		...
		if (isset ($request->form['orderby']) || isset ($request->form['firstletterwhere']) || isset ($request->form['filterwhere'])) {
			$sql = "SHOW FIELDS FROM ".PREFIX_DB."wishlist";
		...
		if($orderby == null)$orderby = "order by data desc";
		...
		$sql = "select * from ".PREFIX_DB."wishlist ".$where.$orderby.$limit;
		//echo $sql;
		$this->arr_wishlist = MySqlDao :: getDati($sql);
		for($x=0;$x<=sizeof($this->arr_wishlist)-1;$x++){
			$this->arr_wishlist[$x]['data'] = date("d/m/Y",strtotime($this->arr_wishlist[$x]['data']));	
		}
		
		$sql = "select count(*) as num_record from ".PREFIX_DB."wishlist ".$where.$orderby;
		$dati = MySqlDao :: getDati($sql);
