	...
	function _checkPostNoPublicActions() {
		...
		if (!isset ($request->form['annulla']) && count($request->form) > 0  && @$request->form['applicazione'] == "dweditor" ) {
			...
			if (isset ($request->form['type'])) {
				...
				$sql = "insert into ".PREFIX_DB."node(node,fk_parent,type,application,icon,last_date,permissions,have_child,fk_user,fk_group)values("."'".$request->form['node']."',"."'".DEFAULT_SAVE_POSITION."',"."'".$request->form['type']."',"."'".$request->form['application']."',"."'".$request->form['icona']."',"."'".date("Y-m-d H:i:s")."',"."'rwx---".$permessi."',"."'".$request->form['have_child']."',"."'".$session->getCurrentUser()."',"."'".$session->getCurrentGroup()."')";
				//echo $sql;die();
