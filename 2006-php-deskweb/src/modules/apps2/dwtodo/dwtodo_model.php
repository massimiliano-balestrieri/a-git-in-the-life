...
/**
 * classe TodoModelDeskWeb
 */
class DWTodoModelDeskWeb {
	...
	function DWTodoModelDeskWeb() {
		...
		if (isset ($request->form['orderby']) || isset ($request->form['firstletterwhere']) || isset ($request->form['filterwhere'])) {
			$sql = "SHOW FIELDS FROM ".PREFIX_DB."todo";
		...
		if($orderby == null)$orderby = "order by scadenza desc";
		...
		$sql = "select * from ".PREFIX_DB."todo ".$where.$orderby.$limit;
		//echo $sql;
		$this->arr_todo = MySqlDao :: getDati($sql);
		for($x=0;$x<=sizeof($this->arr_todo)-1;$x++){
			$this->arr_todo[$x]['scadenza'] = date("d/m/Y",strtotime($this->arr_todo[$x]['scadenza']));	
		}
		
		$sql = "select count(*) as num_record from ".PREFIX_DB."todo ".$where.$orderby;
		$dati = MySqlDao :: getDati($sql);
