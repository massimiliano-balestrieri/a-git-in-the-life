...
/**
 * classe DWGroupsmanagerModelDeskWeb
 */
class DWGroupsmanagerModelDeskWeb {

	/**
	 * array dati weblets
	 */
	var $arr_groupsmanager = array ();
	/**
	 * numero di record
	 */
	var $num_record;
	 ...
	 */
	function DWGroupsmanagerModelDeskWeb() {
		global $request;
		$orderby = null;
		$where = null;
		$limit = null;
		...
		if (isset ($request->form['orderby']) || isset ($request->form['firstletterwhere']) || isset ($request->form['filterwhere'])) {
			$sql = "SHOW FIELDS FROM ".PREFIX_DB."groups";
		...
		$sql = "select id_group,groupname from ".PREFIX_DB."groups ".$where.$orderby.$limit;
		//echo $sql;
		$this->arr_groupsmanager = MySqlDao :: getDati($sql);
		$sql = "select count(*) as num_record from ".PREFIX_DB."groups ".$where.$orderby;
		$dati = MySqlDao :: getDati($sql);
