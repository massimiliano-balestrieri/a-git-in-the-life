 ...
 *
 * 
 * $Id: list_dao.php 379 2008-01-08 16:51:16Z maxbnet $
 * $LastChangedDate: 2008-01-08 17:51:16 +0100 (mar, 08 gen 2008) $
 * $LastChangedRevision: 379 $
 * $LastChangedBy: maxbnet $
 * $HeadURL: https://maxlist.svn.sourceforge.net/svnroot/maxlist/trunk/maxlist/app/models/dao/list_dao.php $
 * 
 * $Author: maxbnet $
 * $Date: 2008-01-08 17:51:16 +0100 (mar, 08 gen 2008) $
 */
...
class ListDao extends ModuleDao{
	...
	public function get($id){
		...
		$list = $this->db->sql_fetch_array_query("SELECT * FROM {$this->table} where id = {$id}");
	...
	public function get_lists(){//TODO: method in user_model. delete?
		...
		$this->_join_listuser->fix_listuser();
	 	return $this->db->sql_query("SELECT * FROM {$this->table} order by listorder");
	}
	//POST list
	...
	public function get_order($key){
		...
		$rec = $this->db->sql_fetch_array_query($sql);
		return isset($rec[1])? $rec : false;
	}
	//call by listmodel 
	public function is_owner($key){
		$owner = $this->db->sql_fetch_array_query(sprintf('select owner from %s where id = %d',$this->table,$key));
		return isset($owner[0]) ? $owner[0] : false; 
	}
	
	public function insert(){
		$params = $this->_params['list'];
		$num_cat = $this->db->sql_fetch_row_query("select listorder from ".$this->table." order by listorder desc");
	...
	public function delete($id){
		$list = $this->db->sql_fetch_array_query("SELECT name FROM $this->table where id = '".$id."'");
	...
	public function get_lists_active(){//TODO: method in user_model. delete?
		$lists = array();
		return $this->db->sql_query("SELECT * FROM {$this->table} where active = 1 order by listorder");
	}
	//model :: members
	public function get_other_lists($id){
		$subselect = "";
		return $this->db->sql_query("select id,name from {$this->table} $subselect");
		
	}
	//members
	public function get_list_name($id){
		$req = $this->db->sql_fetch_row_query(sprintf('select name from %s where id = %d',$this->table,$id));
	...
	private function _check_owner($id){
		global $APP;
		$list = $this->db->sql_fetch_array_query("SELECT owner,name FROM $this->table where id = '".$id."'");
