 ...
 *
 * 
 * $Id: members_dao.php 393 2008-01-18 18:09:49Z maxbnet $
 * $LastChangedDate: 2008-01-18 18:09:49 +0000 (Fri, 18 Jan 2008) $
 * $LastChangedRevision: 393 $
 * $LastChangedBy: maxbnet $
 * $HeadURL: file:///svn/p/maxlist/code/trunk/maxlist/app/models/dao/members_dao.php $
 * 
 * $Author: maxbnet $
...
class MembersDao extends ModuleDao{
	...
	public function get_page_members($id){
		...
		$where = "{$this->table}.listid = $id";
		$order = array("confirmed desc","email");
		//die('DAO->get_page_members');
		$select = $this->db->select()
						   ->from($this->table)
						   ->join($this->db->get_table('user'),
        						  $this->db->get_table('user') .'.id = '.$this->table.'.userid')
						   ->where($where)
						   ->order($order);
		
		
		$result = $this->db->get_select_page($select, $pg);
		//print_r($result);die;
		$count = $this->db->count();
		$member = array();
		if(is_array($result))
	...
	public function subscribe($id,$params,$context = false){
		...
		if ($subselect == "") {
			...
			$res = $this->db->query("select id from {$this->db->get_table("list")} $subselect_where");
			foreach ($res as $row) {
	...
	public function get_listuser_count($id){
		$count = $this->db->fetch_query("select count(*) from {$this->table} where listid = {$id} ");
	...
	public function get_array_memberships($id){ //used by update & insert user. - 
		return $this->db->query("select listid, name from {$this->table},{$this->db->get_table("list")} where listid = {$this->db->get_table("list")}.id and userid = $id");
	}
	...
	private function _get_messagecount($userid,$id){
		 $res = $this->db->fetch_query("select count(*) from " .
