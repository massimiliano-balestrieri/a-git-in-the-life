...
class DHTML_Calendar {
	...
	function return_input_field($cal_options = array(), $field_attributes = array()) {
        $id = $this->_gen_id();
        $attrstr = $this->_make_html_attr(array_merge($field_attributes,
                                                      array('id'   => $this->_field_id($id),
                                                            'type' => 'text',
        ...
        #$html .= '<img align="middle" border="0" src="' . $this->calendar_lib_path . 'img.gif" alt="" />';
		#$html .= '</a>';
		$html .= '<img class="csilist_trigger_jscalendar" id="'. $this->_trigger_id($id) . '" src="' . $this->calendar_lib_path . 'img.gif" alt="apri calendario" />';
		
        $options = array_merge($cal_options,
                               array('inputField' => $this->_field_id($id),
                                     'button'     => $this->_trigger_id($id)));
        $html .=  $this->_make_calendar($options);
        return $html;
    }
    /// PRIVATE SECTION
